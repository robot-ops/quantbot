...
class TelegramNotificationService(NotificationService):
    ...
    def update_credentials(self, token: str, chat_id: str):
        ...
        if self.is_configured():
            try:
                ...
                logger.warning(f"Failed to run command registration inside event loop, running sync: {e}")
                self.register_commands()
    ...
    def register_commands(self) -> bool:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                logger.error(f"Failed to register commands: {res.text}")
                return False
        except Exception as e:
    ...
    async def poll_updates(self, bot_strategy):
        """Asynchronously polls incoming Telegram commands for remote control operations."""
        self.register_commands()
        offset = 0
        while True:
            if not self.is_configured():
